...
namespace HRConfig.Controller
{
  public class Actions
  {
    ...
    public async Task<ActionModel> DoPostPhoto(string caminho, string description)
    {
            ActionModel dataAction = new ActionModel
            {
                Type = "Post Photo",
            ...
            try
            {
                ...
                {
                    dataAction.Status = 1;
                    dataAction.Response = $"[+] {dataAction.Type} | Status: Success | MediaID: {post.Value.Pk}";
                } else
                {
                    dataAction.Response = $"[+] {dataAction.Type} | Status: Failed | Error: {post.Info.Message}";
                }
            }catch (Exception ex)
            {
                dataAction.Response = $"[+] {dataAction.Type} | Status: Failed | Error: {ex.Message}";
    ...
    public async Task<ActionModel> DoPostVideo(string c_video, string c_thumbnail, string description)
    {
        ActionModel dataAction = new ActionModel
        {
            Type = "Post Video",
